    ...
    render(): string {
        return `
            <section class="flex flex-col items-center p-8 gap-6">
                <h2 class="text-white font-2xl font-semibold"> Leaderboard </h2>
                <div class="flex gap-4 items-end p-4">
                    ${new Podium('#B0B3C3', 150)}    
                    ${new Podium('#D7C33C', 200)}    
                    ${new Podium('#C67B4E', 150)}    
                </div>
                <hr class="w-screen bg-gradient-to-r from-background from-30% via-white to-70% via-background h-[1px] border-0" />
